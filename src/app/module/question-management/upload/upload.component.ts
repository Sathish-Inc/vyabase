...
import {
  Attachment,
  QuestionManagementHistory,
} from "../question-management-history.model";
...
export class UploadComponent implements OnInit {
  ...
  onSave() {
    ...
    for (const data of this.dataList) {
      const questionManagement: QuestionManagement = QuestionManagementObjectBuilder.upload(
        data
      );
    ...
    this.qmService.createQuestionHistory(history).subscribe(() => {
  ...
  getExportCsv() {
    ...
    if (this.invalidData.length > 0) {
      ...
      const download = new Angular5Csv(data, "Customer-invalid-data", options);
